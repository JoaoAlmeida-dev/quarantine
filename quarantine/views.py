...
def apagarcomentario(request, grupo_id, pub_id, com_id):
    if is_authenticated(request):
        ...
        if candeletecom:
            com.delete()
            # return publicacao(request, grupo_id, pub_id) return render(request, 'quarantine/publicacao.html',
            # {'grupo': grupo, 'pub': pub, 'candeletepub': candeletepub})
            ...
            return render(request, 'quarantine/publicacao.html', {'grupo': grupo,
                                                                  'pub': pub, 'error_message': "Não é admin do grupo "
                                                                                               "ou autor do "
                                                                                               "comentario!!"})
